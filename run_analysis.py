...
"""
Complete House Price Prediction Pipeline for King County Dataset

This script implements the full machine learning workflow:
1. Load the dataset
...
7. Improve models with feature selection and hyperparameter tuning
...
print("Date can be probably dropped (post dates, just one year thus temporal dynamics isn't available)")
print("No NaNs present")
print("Half of yr_renovated values are 0 which is probably 'not known'")
...
print("Several features show strong correlation and might be excluded")
print("Price correlates with size related features and not so strongly with the rest")
...
print("Most columns have densely distributed values except price and sqft_lot which have more dispersion to the right, especially price")
...
for i in range(len(upper_triangle.columns)):
    for j in range(len(upper_triangle.columns)):
        if i != j and upper_triangle.iloc[i, j] > 0.8:
            ...
            corr_with_target_i = abs(df[upper_triangle.index[i]].corr(df["price"]))
            corr_with_target_j = abs(df[upper_triangle.columns[j]].corr(df["price"]))
            if corr_with_target_i < corr_with_target_j:
                features_to_drop.add(upper_triangle.index[i])
            else:
                features_to_drop.add(upper_triangle.columns[j])

print(f"\nFeatures to be dropped due to high correlation: {list(features_to_drop)}")
...
models_and_results = {
    ...
    "KNN": {
        "model": knn,
        "features": X.columns,
        "R2_test": r2_score(y_test, knn.predict(X_test)),
...
print("Key findings:")
print(f"1. Best model: {final_model_name}")
